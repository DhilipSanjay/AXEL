...
where poll.pollhostid=users.userid and poll.pollid=polloption.pollid group by polloption.pollid,polloption.choiceid having pollid=(select pollid from poll order by heldon desc,pollid desc limit 1)";

$total="select pollid,count(voterid) as totalcount from vote natural join poll group by pollid order by heldon desc,pollid desc limit 1"; //to get total no of votes for a poll to set percentage
